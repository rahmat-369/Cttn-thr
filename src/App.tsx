...
import { 
  ...
  RefreshCw, Plus, Search, Star, Calendar, Edit, Trash2, 
  MessageCircle, Info, X, ArrowLeft, Send, Shuffle,
  Instagram, Music, Github, Home, Download, RotateCcw,
  Code2, Cpu, Database, Zap, ArrowRight
} from 'lucide-react';
...
interface THRStats { totalDiterima: number; totalDiberikan: number; rataRata: number; kategoriTerbanyak: string; }
interface THRAppData { settings: { welcomeDismissed: boolean }; thrMasuk: THRMasuk[]; thrKeluar: THRKeluar[]; stats: THRStats; }
...
// ==========================================
const STYLES = `
  @import url('https://fonts.googleapis.com/css2?family=Amiri:wght@400;700&family=Manrope:wght@400;700;800&family=Playfair+Display:wght@700;900&display=swap');
  :root { --green: #0A4D2E; --gold: #C49A2B; --cream: #F9F1E0; }
  body { background-color: var(--cream); font-family: 'Manrope', sans-serif; overflow-x: hidden; -webkit-tap-highlight-color: transparent; }
  .font-heading { font-family: 'Playfair Display', serif; }
  .font-arabic { font-family: 'Amiri', serif; }
  
  .glass-panel { background: rgba(249, 241, 224, 0.65); backdrop-filter: blur(24px); border: 1px solid rgba(255, 255, 255, 0.6); }
  ...
  .glow-avatar-rhmt { box-shadow: 0 0 50px 10px rgba(59, 130, 246, 0.4); }
...
const aiService = {
  ...
  chat: async (msg: string, d: THRAppData) => new Promise<string>(res => setTimeout(() => {
    ...
    if (msg.toLowerCase().includes("habis")) res(`Sisa THR Anda ${formatRp(s)}. Segera amankan 20% dari sisa tersebut untuk tabungan darurat.`);
    else if (msg.toLowerCase().includes("hemat")) res(`Saran alokasi: 40% Kebutuhan Lebaran, 30% Kewajiban, 20% Tabungan, 10% Sedekah.`);
    else res(`Sistem mendeteksi saldo bersih Anda ${formatRp(s)}. Ada rencana keuangan yang ingin dianalisis?`);
  }, 1000))
};
...
export default function App() {
  const [data, setData] = useState<THRAppData>(() => { try { const s = localStorage.getItem('thr-tracker-v3'); return s ? JSON.parse(s) : DEFAULT_DATA; } catch { return DEFAULT_DATA; } });
  ...
  useEffect(() => {
    ...
    const cats = data.thrMasuk.map(i => i.kategori).reduce((acc: any, cat: string) => { acc[cat] = (acc[cat] || 0) + 1; return acc; }, {});
    const sortedCats = Object.keys(cats).sort((a, b) => cats[b] - cats[a]);
    localStorage.setItem('thr-tracker-v3', JSON.stringify({ ...data, stats: { totalDiterima, totalDiberikan, rataRata: data.thrMasuk.length ? totalDiterima / data.thrMasuk.length : 0, kategoriTerbanyak: sortedCats.length ? sortedCats[0] : '-' } }));
  }, [data.thrMasuk, data.thrKeluar]);

  const handleExport = () => { const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `Backup-THR-1446H.json`; a.click(); setIsSidebarOpen(false); };
  const handleReset = () => { if(window.confirm("Yakin ingin menghapus seluruh data THR?")) { setData(DEFAULT_DATA); localStorage.removeItem('thr-tracker-v3'); setIsSidebarOpen(false); } };
    ...
    <div className="min-h-screen islamic-pattern relative flex flex-col text-[#2C3E2F]">
      ...
      <AnimatePresence>
        {!data.settings.welcomeDismissed && (
          <motion.div exit={{ opacity: 0, scale: 1.1, filter: 'blur(10px)' }} transition={{ duration: 0.5 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-2xl">
            <motion.div initial={{ y: 50, scale: 0.9 }} animate={{ y: 0, scale: 1 }} transition={{ type: "spring", damping: 25 }} className="bg-white/95 backdrop-blur-3xl rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl flex flex-col items-center text-center border border-white">
              <motion.img animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }} src={IDUL_FITRI_ICON} alt="Idul Fitri" className="w-28 h-28 mb-6 drop-shadow-xl" />
              <h2 className="font-arabic text-3xl text-[#C49A2B] mb-2">اَلسَّلَامُ عَلَيْكُمْ</h2>
              <h3 className="font-heading text-2xl font-black text-[#0A4D2E] mb-4 tracking-tight">THR TRACKER</h3>
              <p className="text-[#2C3E2F] mb-8 text-sm font-medium opacity-80">Catat, kelola, dan amankan arus kas hari raya Anda dengan asisten AI.</p>
              <button onClick={() => setData(p => ({ ...p, settings: { ...p.settings, welcomeDismissed: true } }))} className="w-full py-4 bg-[#0A4D2E] text-white rounded-2xl font-black transition-all hover:bg-[#C49A2B] hover:shadow-lg active:scale-95 flex items-center justify-center gap-2">
                MULAI APLIKASI <ArrowRight size={20} />
              </button>
            </motion.div>
          </motion.div>
        )}
      ...
      <header className="glass-panel sticky top-0 z-40 px-4 py-4 md:px-8 border-b border-white">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(true)} className="p-2.5 bg-white/80 hover:bg-white rounded-2xl shadow-sm text-[#0A4D2E]">
              <Menu size={20} />
            </button>
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsSidebarOpen(false)} className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50" />
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25 }} className="fixed top-0 left-0 h-full w-[85%] max-w-[320px] bg-white shadow-2xl z-50 flex flex-col">
              <div className="p-8 bg-[#0A4D2E] text-white rounded-br-[3rem] relative overflow-hidden">
                ...
                <div className="relative z-10 flex justify-between items-start">
                  ...
                  <button onClick={() => setIsSidebarOpen(false)} className="p-2 bg-white/10 rounded-full hover:bg-white/30"><X size={20}/></button>
                </div>
              </div>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-8 relative z-10 flex flex-col">
        <AnimatePresence mode="wait">
          {route === 'dashboard' && <Dashboard key="dash" data={data} setData={setData} openModal={(t:any, id=null) => setModalState({isOpen: true, type: t, editId: id})} navigate={setRoute} />}
      ...
      <AnimatePresence>
        {modalState.isOpen && (
          <TransactionModal config={modalState} data={data} onClose={() => setModalState({isOpen: false, type: null, editId: null})} onSave={(newData:any) => {
              if (modalState.type === 'masuk') {
                if (modalState.editId) setData(p => ({...p, thrMasuk: p.thrMasuk.map(t => t.id === modalState.editId ? newData : t)}));
                else setData(p => ({...p, thrMasuk: [newData, ...p.thrMasuk]}));
              } else {
                setData(p => ({...p, thrKeluar: [newData, ...p.thrKeluar]}));
              }
...
const SidebarItem = ({ icon, label, onClick, active, danger }: any) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all font-bold text-sm ${active ? 'bg-[#0A4D2E] text-white shadow-lg translate-x-2' : danger ? 'text-red-500 hover:bg-red-50' : 'text-gray-600 hover:bg-gray-100'}`}>
...
const Dashboard = ({ data, setData, openModal, navigate }: any) => {
  ...
  const [aiQuoteStr, setAiQuoteStr] = useState("Memuat pesan...");
  const [searchTerm, setSearchTerm] = useState("");
  
  useEffect(() => { aiService.generateQuote(data).then(setAiQuoteStr); }, [data]);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8 pb-10">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div whileHover={{ scale: 1.01 }} className="glass-card rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden">
          <div className="absolute -right-10 -top-10 w-48 h-48 bg-[#C49A2B]/20 rounded-full blur-3xl" />
          ...
          <p className="text-4xl md:text-5xl font-black text-[#0A4D2E] mb-10 tracking-tighter">{formatRp(data.stats.totalDiterima)}</p>
          <div className="flex gap-4 relative z-10">
            <button onClick={() => openModal('masuk')} className="flex-1 bg-[#0A4D2E] text-white py-4 rounded-2xl font-black flex justify-center items-center gap-2 hover:bg-[#C49A2B] hover:shadow-lg transition-all active:scale-95"><ArrowDownCircle size={20}/> Masuk</button>
            <button onClick={() => openModal('keluar')} className="flex-1 bg-white text-[#0A4D2E] border-2 border-gray-100 py-4 rounded-2xl font-black flex justify-center items-center gap-2 hover:bg-gray-50 transition-all active:scale-95"><ArrowUpCircle size={20}/> Keluar</button>
          </div>
        </motion.div>
        ...
        <motion.div whileHover={{ scale: 1.01 }} className="glass-card rounded-[2.5rem] p-8 md:p-10 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-heading text-sm font-black text-gray-500 tracking-widest uppercase">AI Insight</h3>
            </div>
          ...
          <button onClick={() => aiService.generateQuote(data).then(setAiQuoteStr)} className="mt-6 self-start bg-white px-5 py-2.5 rounded-full text-[#0A4D2E] font-bold text-sm flex items-center gap-2 hover:bg-gray-50 shadow-sm"><RefreshCw size={16} /> Refresh Insight</button>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] p-6 md:p-10 border border-white shadow-sm">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
              ...
              <div className="relative w-full md:w-auto">
                ...
                <input type="text" placeholder="Cari pemberi..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full md:w-64 pl-12 pr-5 py-4 rounded-2xl bg-white border-2 border-gray-100 focus:border-[#C49A2B] outline-none font-bold text-sm" />
              </div>
            </div>
              ...
              <AnimatePresence>
                {filteredMasuk.length === 0 ? (
                  ...
                  filteredMasuk.map((item: THRMasuk, i: number) => (
                    <motion.div key={item.id} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[2rem] p-6 flex flex-col sm:flex-row justify-between gap-5 border border-gray-100 shadow-sm hover:shadow-lg transition-all">
                      <div>
                        <div className="flex items-center gap-3 mb-3">
                          <span className="font-black text-xl text-[#0A4D2E] uppercase">{item.namaPemberi}</span>
                          <span className="px-3 py-1 bg-[#F9F1E0] text-[#C49A2B] text-[10px] font-black rounded-full uppercase">{item.kategori}</span>
                        </div>
                      ...
                      <div className="flex flex-row sm:flex-col items-end justify-between">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => openModal('masuk', item.id)} className="p-2 text-blue-500 bg-blue-50 hover:bg-blue-100 rounded-xl"><Edit size={16}/></button>
                          <button onClick={() => setData((p:any) => ({...p, thrMasuk: p.thrMasuk.filter((t:any) => t.id !== item.id)}))} className="p-2 text-red-500 bg-red-50 hover:bg-red-100 rounded-xl"><Trash2 size={16}/></button>
                        </div>
                      </div>
                    </motion.div>
                  ))
        ...
        <div className="space-y-6">
          <div className="glass-card rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div className="space-y-4 mb-8">
              <div className="flex justify-between items-center bg-white/60 p-4 rounded-2xl border border-white">
                <span className="text-gray-500 text-xs font-black uppercase">Total Diterima</span>
                <span className="text-[#0A4D2E] font-black">{formatRp(data.stats.totalDiterima)}</span>
              </div>
              <div className="flex justify-between items-center bg-white/60 p-4 rounded-2xl border border-white">
                <span className="text-gray-500 text-xs font-black uppercase">Total Keluar</span>
              ...
              <div className="flex justify-between items-center bg-[#0A4D2E] p-5 rounded-2xl text-white shadow-lg shadow-[#0A4D2E]/30">
                <span className="text-sm font-black uppercase">THR Bersih</span>
            ...
            <div className="bg-white/80 p-5 rounded-2xl border border-white text-center">
               <p className="text-[10px] text-gray-400 uppercase font-black mb-2">Pemberi Terbanyak</p>
          ...
          <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-8 border border-white text-center">
            <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">Mutiara Lebaran</h4>
            <p className="font-arabic text-xl text-[#2C3E2F] mb-6 min-h-[60px]">{QUOTES[quoteIndex]}</p>
...
const Chat = ({ data, navigate }: any) => {
  const [messages, setMessages] = useState<{role: string, content: string}[]>([{ role: 'ai', content: "Halo! Mari rencanakan keuangan Lebaran Anda dengan cermat berdasarkan data THR yang telah terkumpul." }]);
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="flex-1 h-[calc(100vh-140px)] flex flex-col bg-white/90 backdrop-blur-2xl rounded-[3rem] overflow-hidden shadow-2xl border border-white">
      <div className="bg-white px-8 py-5 flex items-center justify-between border-b border-gray-100 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('dashboard')} className="p-2 bg-gray-50 hover:bg-gray-100 rounded-full"><ArrowLeft size={20}/></button>
          <div><h2 className="font-heading font-black text-xl text-[#0A4D2E]">AI ASSISTANT</h2><div className="flex items-center gap-2"><div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"/><p className="text-[10px] text-gray-400 font-bold uppercase">System Online</p></div></div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-gradient-to-b from-transparent to-[#F9F1E0]/30">
        {messages.map((m, i) => (
          <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-5 text-sm md:text-base leading-relaxed font-bold shadow-sm ${m.role === 'user' ? 'bg-[#0A4D2E] text-white rounded-[24px_24px_4px_24px]' : 'bg-white text-[#2C3E2F] border border-gray-100 rounded-[24px_24px_24px_4px]'}`}>{m.content}</div>
          </motion.div>
        ))}
        {isLoading && (<div className="flex justify-start"><div className="bg-white p-5 rounded-[24px_24px_24px_4px] flex gap-2 border border-gray-100"><motion.div animate={{scale:[1,1.5,1]}} transition={{repeat:Infinity, duration:1}} className="w-2 h-2 bg-[#C49A2B] rounded-full"/><motion.div animate={{scale:[1,1.5,1]}} transition={{repeat:Infinity, duration:1, delay:0.2}} className="w-2 h-2 bg-[#C49A2B] rounded-full"/><motion.div animate={{scale:[1,1.5,1]}} transition={{repeat:Infinity, duration:1, delay:0.4}} className="w-2 h-2 bg-[#C49A2B] rounded-full"/></div></div>)}
        <div ref={endRef} />
      </div>
      ...
      <div className="bg-white p-6 border-t border-gray-100">
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2 no-scrollbar">
          {["Sisa THR?", "Tips hemat", "Alokasi Dana"].map(chip => (
             <button key={chip} onClick={() => handleSend(chip)} className="whitespace-nowrap px-5 py-2.5 bg-gray-50 border border-gray-100 text-[#0A4D2E] text-xs font-black rounded-full hover:bg-gray-100 uppercase">{chip}</button>
          ))}
        </div>
        <div className="flex gap-3 relative">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend(input)} placeholder="Tanyakan seputar keuangan..." className="flex-1 bg-gray-50 border-2 border-gray-100 rounded-[2rem] px-6 py-4 focus:border-[#0A4D2E] outline-none font-bold text-gray-700" />
          <button onClick={() => handleSend(input)} disabled={!input.trim()} className="w-16 h-16 bg-[#0A4D2E] text-white rounded-[2rem] flex items-center justify-center hover:bg-[#C49A2B] shadow-lg disabled:opacity-50"><Send size={20} className="ml-1" /></button>
        </div>
      </div>
    </motion.div>
  );
...
// ==========================================
// ABOUT DEVELOPER (Strictly R_hmt ofc)
// ==========================================
const About = ({ navigate }: { navigate: any }) => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-4xl mx-auto space-y-8 pb-10">
    <button onClick={() => navigate('dashboard')} className="flex items-center gap-2 bg-white px-6 py-3 rounded-full text-[#0A4D2E] hover:bg-gray-50 font-black shadow-sm text-sm uppercase w-fit"><ArrowLeft size={18}/> Kembali</button>
    ...
    <div className="glass-card rounded-[3rem] p-8 md:p-16 text-center relative overflow-hidden border-2 border-white shadow-[0_20px_60px_rgba(0,0,0,0.05)]">
      ...
      <div className="relative mb-8 mt-4">
         <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 bg-blue-500/20 rounded-full blur-3xl animate-pulse" />
         <img src={DEV_AVATAR} alt="R_hmt" className="relative w-36 h-36 rounded-full object-cover border-[3px] border-blue-400/50 glow-avatar-rhmt mx-auto z-10 hover:scale-105 transition-transform duration-700" />
      </div>

      <h2 className="font-heading text-4xl md:text-6xl font-black text-[#0A4D2E] mb-2">R_hmt ofc</h2>
...
const TechBadge = ({icon, label}:any) => (<div className="flex items-center gap-2 bg-white/80 px-4 py-2 rounded-xl text-xs font-black text-[#0A4D2E] uppercase">{icon} {label}</div>);
const SocialCard = ({ icon, title, link }: any) => (<a href={link} target="_blank" rel="noreferrer" className="bg-white/90 p-5 rounded-[2rem] border border-white hover:shadow-xl hover:-translate-y-2 transition-all group flex items-center gap-4"><div className="p-4 bg-gray-50 text-[#0A4D2E] rounded-2xl group-hover:bg-[#0A4D2E] group-hover:text-white transition-colors">{React.cloneElement(icon, { size: 24 })}</div><h4 className="font-black text-[#0A4D2E] text-sm uppercase">{title}</h4></a>);
...
const TransactionModal = ({ config, data, onClose, onSave }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-xl">
      <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#FEF9E7] rounded-[3rem] p-8 w-full max-w-lg shadow-[0_30px_60px_rgba(0,0,0,0.3)] relative border-4 border-white max-h-[90vh] overflow-y-auto">
        <button type="button" onClick={onClose} className="absolute right-6 top-6 p-3 bg-white rounded-full text-gray-400 hover:text-red-500 shadow-sm"><X size={20}/></button>
        <h2 className="font-heading text-2xl font-black text-[#0A4D2E] mb-8">{config.editId ? 'Edit Data' : `Catat THR ${isM ? 'Masuk' : 'Keluar'}`}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Jumlah (Rp)</label><div className="relative"><span className="absolute left-6 top-1/2 -translate-y-1/2 font-black text-gray-400">Rp</span><input required type="number" min="1" value={formData.jumlah} onChange={e => setFormData(p => ({...p, jumlah: e.target.value}))} className="w-full bg-white border-2 border-white rounded-[2rem] pl-16 pr-6 py-4 focus:border-[#C49A2B] outline-none font-black text-3xl text-[#0A4D2E] shadow-sm"/></div></div>
          {isM && (<div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Rating Keikhlasan</label><div className="flex gap-2 bg-white p-4 rounded-[2rem] shadow-sm w-fit">{[1,2,3,4,5,6].map(star => (<Star key={star} size={32} onClick={() => setFormData(p => ({...p, rating: star as any}))} className={`cursor-pointer transition-all ${star <= formData.rating ? "fill-[#C49A2B] text-[#C49A2B] scale-110" : "text-gray-100"}`} />))}</div></div>)}
          <div className="grid grid-cols-2 gap-4">
            ...
            {isM ? (<div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Kategori</label><select value={formData.kategori} onChange={e => setFormData(p => ({...p, kategori: e.target.value}))} className="w-full bg-white border-2 border-white rounded-[2rem] px-5 py-4 focus:border-[#C49A2B] outline-none font-bold text-sm shadow-sm"><option>Keluarga</option><option>Tetangga</option><option>Teman</option><option>Kerabat</option><option>Lainnya</option></select></div>) : (<div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Hubungan</label><input required type="text" placeholder="Cth: Keponakan" value={formData.hubungan} onChange={e => setFormData(p => ({...p, hubungan: e.target.value}))} className="w-full bg-white border-2 border-white rounded-[2rem] px-5 py-4 focus:border-[#C49A2B] outline-none font-bold text-sm shadow-sm"/></div>)}
          </div>
          <div className="flex gap-4 pt-6">
            <button type="button" onClick={onClose} className="w-1/3 py-4 rounded-[2rem] bg-gray-100 text-gray-500 font-black uppercase text-xs hover:bg-gray-200">Batal</button>
            <button type="submit" className="w-2/3 py-4 rounded-[2rem] bg-[#0A4D2E] text-white font-black text-sm uppercase hover:bg-[#C49A2B] active:scale-95 transition-all">Simpan</button>
          </div>
        </form>
      </motion.div>
    </div>
